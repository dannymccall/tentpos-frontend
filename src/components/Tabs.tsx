...
export default function Tabs({
  ...
  const variantClasses = {
    ...
    segmented: {
      trigger:
        "flex-1 text-center rounded-md px-4 py-2 text-sm font-medium transition-all data-[state=active]:bg-[#0f172b] data-[state=active]:text-white text-slate-600 hover:bg-slate-100",
      list: "bg-white p-1 flex w-full",
    },
  };

  // Filter tabs based on permission
  const visibleTabs = tabs.filter((tab) => canView(tab.code!));
